...
export class AIManager {
  ...
  private chooseBestDie(dice: Die[]): Die | null {
    ...
    // If threat level is very high, prioritize defense (increased threshold from 3 to 5)
    if (threatLevel > 5) {
      console.log(
        "Very high threat to blue flag detected! Prioritizing defense."
      );
      ...
      diceWithPaths.sort((a, b) => {
        ...
        if (a.interceptionPaths.length > 0 && b.interceptionPaths.length === 0)
          return -1;
        if (a.interceptionPaths.length === 0 && b.interceptionPaths.length > 0)
          return 1;
    ...
    // If threat level is medium, still mostly focus on offense (increased threshold from 1 to 3)
    if (threatLevel > 3) {
      console.log(
        "Medium threat to blue flag detected. Still mostly focusing on offense."
      );

      // Only 25% chance to play defensively when under medium threat (reduced from 50%)
      if (Math.random() < 0.25) {
        // Sort by best defensive position
        diceWithPaths.sort((a, b) => {
          if (
            a.interceptionPaths.length > 0 &&
            b.interceptionPaths.length === 0
          )
            return -1;
          if (
            a.interceptionPaths.length === 0 &&
  ...
  private gridPositionFromWorld(worldPos: THREE.Vector3): {
    x: number;
    z: number;
  } {
    return {
  ...
  private worldPositionFromGrid(gridPos: {
    x: number;
    z: number;
  }): THREE.Vector3 {
  ...
  private isPositionOccupiedBySameColor(
    ...
    return diceArray.some(
      (die) =>
        Math.abs(die.mesh.position.x - worldPos.x) < 0.1 &&
        Math.abs(die.mesh.position.z - worldPos.z) < 0.1
    );
  }
    ...
    while (openSet.length > 0 && iterations < maxIterations) {
      ...
      for (const neighbor of neighbors) {
        ...
        if (!existingNeighbor) {
          ...
          neighbor.h = this.calculateManhattanDistance(
  ...
  private getNeighbors(
    node: PathNode,
    goal: { x: number; z: number },
    allowRedDiceInterception: boolean
  ): PathNode[] {
  ...
  private calculateThreatToBlueFlag(): number {
    ...
    this.redDice.forEach((die) => {
      ...
      // Closer dice pose a threat, but we're less concerned (reduced threat values)
      if (distance <= 2) {
        threatLevel += 2; // Very close - still a threat but less severe
      } else if (distance <= 4) {
        threatLevel += 1; // Moderately close - minor threat
      } else if (distance <= 6) {
        threatLevel += 0.5; // Somewhat close - very minor threat
      }

      // Higher value dice pose a threat, but less than before
      threatLevel += die.topFace / 12; // Add 0.08 to 0.5 based on die value (halved from before)
    });
  ...
  private chooseBestDirection(die: Die): THREE.Vector3 | null {
    ...
    const directionsWithScores = validDirections.map((dir) => {
      ...
      // If in defensive mode, prioritize moving toward blue flag but still consider offense
      if (isDefensiveMode) {
        ...
        const wouldBlockRedPath = this.redDice.some((redDie) => {
          ...
          if (this.calculateDistanceToBlueFlag(redDie) <= 6) {
            ...
            // If new position is roughly on the path from red die to blue flag
            return redToNew + newToBlue - redToBlue < 3;
          }
          return false;
        });

        // If this move would block a red die's path, give it a bonus (reduced from 50 to 30)
        if (wouldBlockRedPath) {
          score -= 30;
        }

        // Also consider distance to red flag even in defensive mode (aggressive behavior)
        const distanceToRedFlag = this.calculateManhattanDistance(
          this.gridPositionFromWorld(newPos),
          this.gridPositionFromWorld(this.redFlag.mesh.position)
        );

        // Add a significant offensive component to the score (lower is better)
        score += distanceToRedFlag * 0.5; // Weight of 0.5 means it's still important but not as much as defense
      }
      // Otherwise, prioritize offense
        ...
        score =
          this.calculateManhattanDistance(
            this.gridPositionFromWorld(newPos),
            this.gridPositionFromWorld(this.redFlag.mesh.position)
          ) * 2; // Double the importance of getting to the red flag (more aggressive)
      }
